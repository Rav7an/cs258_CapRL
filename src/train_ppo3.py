...
OUTPUT_DIR = "output/ppo_qwen2vl_v3"
...
RL_STEPS = 4000        # Adjusted for ~4 hours training (4000 * 3.5s approx 3.9 hours)
...
EVAL_EVERY = 250           # Run eval every N steps
...
def compute_reward(llm_model, llm_tokenizer, caption: str, question: str, correct_label: str) -> float:
    ...
    reward = 1.0 if pred_label == correct_label else 0.0
    
    # Length penalty: if caption is too short (< 10 words), penalize
    # This prevents the model from collapsing to empty or very short strings
    # that might accidentally trigger a correct guess or just be degenerate.
...
def main():
    ...
    # Limit dataset to 1000 images as requested
    items = items[:1000]
    print(f"Dataset reduced to {len(items)} samples.")
    ...
    try:
        while step < RL_STEPS:
            ...
            # KL Penalty
            kl = logp_old - logp
            kl_coef = 0.05  # Reduced from 0.1 to allow more deviation if needed, but keep it to prevent collapse
